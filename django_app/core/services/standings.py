...
S2 port of ``backend/src/services/standings.py``. The **algorithm body is
reused VERBATIM** — the points→position table, ``position_from_points``, the
per-event scoring fan-out, the percentile ``total_inverse_position`` sum, the
``aggregate_2025`` drop-worst rule, and the 6-link tiebreaker sort key are
byte-identical to the FastAPI source (pinned by the ported
``test_standings_tiebreakers.py`` / ``test_standings_inverse_position.py``).

ORM translation (the ONLY change vs the FastAPI source)
-------------------------------------------------------
Only the SQLAlchemy data-fetch layer is rewritten to the Django ORM; the
...
* ``select(Event).where(Event.season_id == season.id)
  ...
  — same WHERE, same ``ORDER BY sort_order, id`` (observable in the response
  ``events`` array; byte-parity preserved).

* ``select(Rider).where(Rider.category_id == category.id)
  .options(selectinload(Rider.results))``
  →  ``Rider.objects.filter(category_id=category.id).prefetch_related(
  Prefetch("results", EventResult.objects.order_by("event__sort_order",
  "event_id", "id")))``
  — Django ``prefetch_related`` is the exact analogue of SQLAlchemy
  ``selectinload``: ONE extra query batches every related ``EventResult``
  via ``WHERE rider_id IN (...)`` (NOT a per-rider query). This is the
  **No-N+1 acceptance criterion**: the whole leaderboard is a small fixed
  number of queries regardless of rider/result count.

  KNOWN, DOCUMENTED, PARITY-SAFE DIVERGENCE (surfaced to I1 — not silently
  absorbed). The FastAPI oracle's ``selectinload(Rider.results)`` issues
  ``SELECT … FROM event_result WHERE rider_id IN (…)`` with **NO
  ``ORDER BY``** (verified against SQLAlchemy's emitted SQL), so the
  per-row ``events`` array order in the oracle's JSON is whatever Postgres
  heap/physical order ``bgx_oracle`` happens to have — an *unspecified
  storage artifact*, NOT a data/behavior contract: it is stable per-DB but
  differs between two "identically seeded" DBs (``bgx_oracle``'s heap is
  scrambled relative to ``id`` order; ``bgx_django``'s is not), and the
  Astro frontend never renders ``row.events`` (the leaderboard table reads
  only ``final_position``/``rider``/``total_points``/``races_participated``/
  ``best_position``), and the ``openapi-typescript`` client types it as an
  unordered array. Reproducing one DB's arbitrary heap order in the other
  is impossible *and meaningless*. The honest, parity-faithful resolution
  (mirrors F3's ``Content-Type``-charset + X1 catch-all documented
  divergences): the Django app pins a **deterministic, semantically
  meaningful** order — race-calendar order (``event.sort_order``, then
  ``event_id``, then ``id`` as the stable final break) — and the S2 parity
  assertion normalizes ONLY the per-row ``events`` array order before the
  byte compare. With that one non-semantic divergence factored out, every
  observable field (``final_position``, ``total_points``, ``best_position``,
  ``worst_*``, every per-event ``points``/``position``, rider, slug, the
  full tiebreaker ranking) is byte-identical to the oracle (proven by
  ``parity_rig``).

Signature change (service-API contract for S3/S4/S7)
----------------------------------------------------
...
service API drops that parameter:

  * ``get_standings(session, season, category)``  →  ``get_standings(season, category)``
  * ``events_for_season(session, season)``        →  ``events_for_season(season)``
  * ``position_from_points(points)``              →  unchanged (pure fn)

This is the exact, documented service surface S3/S4/S7 import from
``core.services.standings`` (Lane D). The OBSERVABLE behavior — the returned
``StandingsRow`` list, ordering, every numeric field — is parity-identical.
...
def get_standings(season: Season, category: Category) -> list[StandingsRow]:
    events = list(
    ...
    # ONE batched results query (the selectinload analogue → No-N+1), but
    # with a DETERMINISTIC, semantically meaningful order: race-calendar
    # order (event.sort_order, then event_id, then id). The oracle's
    # selectinload emits NO ORDER BY (its per-row `events` order is an
    # unspecified Postgres heap artifact, not a contract — see module
    # docstring). Pinning calendar order here makes the new app's output
    # stable + meaningful; the S2 parity assertion normalizes this one
    # non-semantic, documented divergence (surfaced to I1).
    riders = list(
        Rider.objects.filter(category_id=category.id).prefetch_related(
            Prefetch(
                "results",
                queryset=EventResult.objects.order_by(
                    "event__sort_order", "event_id", "id"
                ),
            )
        )
    )
    ...
    for ev in events:
        cat_results = [
            er for r in riders for er in r.results.all() if er.event_id == ev.id
        ]
        if cat_results:
...
def events_for_season(season: Season) -> list[Event]:
    return list(
        Event.objects.filter(season_id=season.id).order_by(
            "sort_order", "id"
        )
