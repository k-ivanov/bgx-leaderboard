...
- [ ] **🔒 HARD DATA-LOSS GATE (F2 deferral, user decision A 2026-05-17):** F2 was merged to the non-prod integration branch with its sandbox schema-clone rehearsal green, but the **production-DATA-clone rehearsal was deferred to here**. Before ANY prod `migrate`/DNS: run `django_app/core/MIGRATION_REHEARSAL.md §3` against a freshly restored **production** clone; verify the 8 domain tables get zero schema/data drift, Django contrib tables created, `core.0001_initial` FAKED, idempotent re-run; **explicit human sign-off required**. This gate is blocking and non-skippable — it is the only place the F2 data-loss risk is actually closed.
